...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (m.isGroup && !m.isNewsletter) {
      ...
      if (isMutegc) {
        try {
          if (isMutegc(m.chat, db)) {
            const _prefix = config.command?.prefix || ".";
            const _body = (m.body || "").trim().toLowerCase();
            const _unmuteAliases = ["unmutegc", "unmutegrup", "unmutebot", "unblockbot", "unlockbot"];
            const _isUnmuteCmd = _unmuteAliases.some(
              (a) => _body === `${_prefix}${a}` || _body.startsWith(`${_prefix}${a} `)
            );
            if (!_isUnmuteCmd) return;
          }
        } catch (e) { }
      }
